...
public class ResumeExceptionHandler {
	...
	public ResponseEntity<ErrorResponse> handleResumeNotFoundEntity(HttpServletRequest http,
			ResumeNotFoundException ex) {
		ErrorResponse error = new ErrorResponse(HttpStatus.NOT_FOUND.value(), "Resume Not Found", ex.getMessage(),
				LocalDateTime.now(), http.getRequestURI());
